...
class Trainer:
    ...
    def __init__(self,
                 ...
                 test_dataloader: AbsIterFactory,
    ...
    def run(self) -> None:
        ...
        for name, param in model.named_parameters():
            if "clip_encoder" in name or "visual_adapter" in name:
                param.requires_grad = False
            else:
                param.requires_grad = True
        logging.info("------------freeze clip encoder----------------")
    ...
    @torch.no_grad()
    def evaluate(self,merge_method: int=3) -> None:
        model=self.model
        model.eval()
        test_dataloader=self.test_dataloader
        iterator=test_dataloader.build_iter(0)
        total_wer_bias,total_wer_att,total_wer_merge=0., 0., 0.
        weights=0
        count=0
        for (_, batch) in iterator:
            batch = to_device(batch, "cuda" if self.trainer_options.ngpu > 0 else "cpu")
            retval = model(**batch,is_test=True,merge_method=merge_method)
            weight=retval[2].item()

            wer_bias=retval[1]["wer_bias"].item()
            wer_att=retval[1]["wer_att"].item()
            wer_merge=retval[1]["wer_merge"].item()
            total_wer_bias += wer_bias * weight
            total_wer_att += wer_att * weight
            total_wer_merge += wer_merge * weight
            weights += weight
            count+=1
            logging.info("index:{} wer_att:{},wer_bias:{} wer_merge:{}".format(count,round(wer_att,5),round(wer_bias,5),round(wer_merge,5)))       
        avg_wer_bias = total_wer_bias / weights
        avg_wer_att = total_wer_att / weights
        avg_wer_merge = total_wer_merge / weights
        logging.info("average: wer_att:{},wer_bias:{} wer_merge:{}".format(round(avg_wer_att,5),round(avg_wer_bias,5),round(avg_wer_merge,5)))
